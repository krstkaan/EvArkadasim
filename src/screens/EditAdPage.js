...
const EditAdPage = ({ route }) => {
        ...
        const fetchAdDetails = async () => {
            ...
            try {
                ...
                const response = await fetch(
                    'https://roomiefies.com/app/getmyilandetails.php', // URL güncellendi
                    {
                        method: 'POST',
                        body: formData,
                        headers: {
                ...
                console.log("API'den gelen veri:", data);
                 if (data && data.length > 0) {
                  ...
                  setDescription(ad.description);
                   setRent(String(ad.rent));
                   setSize(String(ad.size));
                   setBinaYasi(String(ad.binayasi));
                   setCinsiyet(String(ad.cinsiyet));
                   setEsya(String(ad.esya));
                    setDaireTipi(String(ad.dairetipi));
                    setIsitmaTuru(String(ad.isitmaturu));
                    setYasAraligi(String(ad.yasaraligi));
                   setSelectedIl(String(ad.selectedIl));
                    setSelectedIlce(String(ad.selectedIlce));
                    setSelectedMahalle(String(ad.selectedMahalle));

                    setImage1Url(`https://roomiefies.com/app/${ad.imageurl1}`);
                    setImage2Url(`https://roomiefies.com/app/${ad.imageurl2}`);
                    setImage3Url(`https://roomiefies.com/app/${ad.imageurl3}`);
                }
    
            } catch (error) {
                console.error('Error fetching ad details:', error);
                setError("İlan detayları alınırken bir sorun oluştu.");
            } finally {
                setSending(false);
            }
        };
  ...
  const Card = ({ title, imageUrl, handleOnpress, handleResimsil }) => {
      if (!imageUrl) {
      return  <View style={styles.cardIlan}>
          <View style={{ height: 120, width: '100%', backgroundColor:'#f0f0f0', borderRadius: 10,  justifyContent: 'center', alignItems: 'center' }}>
  ...
  const sendData = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://roomiefies.com/app/ilanupdate.php', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
        ...
        if (response.data.sonuc === 1) {
             Alert.alert('Başarılı', 'İlanınız düzenleme için onaya gönderildi!');
           setSending(false);
           navigation.goBack()
        } else {
  ...
  if (sending) {
    return <Loading IndicatorMetni='Güncelleniyor...' />;
  }

 const showImageFullScreen = (imageUrl) => {
      if(imageUrl){
      setSelectedImage(imageUrl);
      setResimmodalVisible(true);
      }
    }
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} style={styles.contentContainerStyle}>
      ...
      <Text style={styles.label}>Ürün Resimleri</Text>
       <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
        <View style={styles.cardsRowIlan}>
          <Card
             title="1/3"
             imageUrl={image1 || image1Url}
             handleOnpress={() => pickimage({ hangiimage: 1 })}
             handleResimsil={() => { setImage1(null) ,setImage1Url(null)}}
          />
          <Card
              title="2/3"
              imageUrl={image2 || image2Url}
                handleOnpress={() => { if ((!image1 && !image1Url)) { pickimage({ hangiimage: 1 }) } else { pickimage({ hangiimage: 2 }) } }}
             handleResimsil={() => {setImage2(null) , setImage2Url(null)}}
          />
          <Card
              title="3/3"
                imageUrl={image3 || image3Url}
                 handleOnpress={() => { if ((!image1 && !image1Url)) { pickimage({ hangiimage: 1 }) } else if ((!image2 && !image2Url)) { pickimage({ hangiimage: 2 }) } else { pickimage({ hangiimage: 3 }) } }}
              handleResimsil={() => {setImage3(null) , setImage3Url(null)}}
          />
        </View>
      </ScrollView>
      ...
      <TouchableOpacity onPress={handleSubmit} style={[styles.button, styles.submitButton]}>
        <Text style={styles.buttonText}>İlanı Güncelle</Text>
      </TouchableOpacity>
